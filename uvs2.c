  ...
  int main(void)
{
  ...
  double  data;
  double  sum = 0.0;
  double s1 = 0.0;   //first sum in sqew
  double s2 = 0.0;   //second sum in sqew
  double  mean;
  double  stdev;
  double  min;
  double  max;
  ...
  while (!feof(stdin)) {
    ...
    s1 += diff * diff * diff;   //first sum in one-pass algorithm
    s2 += diff * diff;          //second sum in one pass algorithm
    mean += diff / index;
    if (data > max)
      max = data;
    else if (data < min)
  ...
  if (index > 0) {
    ...
    skew = (s1/index + s2*3*mean/index + 2*mean*mean*mean/index); //partial computation
    skew = skew / (stdev*stdev*stdev);   //complete computation for skew
    q = cbrt(skew);   /// finding cube root for skew
